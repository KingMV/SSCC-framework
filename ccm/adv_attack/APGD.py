...
class APGD(object):
    ...
    def __init__(self, model, norm='Linf', eps=8 / 255, steps=10, n_restarts=1, seed=0, loss='ce', eot_iter=1, rho=.75,
                 verbose=False):
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.norm == 'Linf':
            ...
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0], 1, 1, 1]).cuda().detach() * t / (
                t.reshape([t.shape[0], -1]).abs().max(dim=1, keepdim=True)[0].reshape([-1, 1, 1, 1]))  # nopep8
        elif self.norm == 'L2':
            t = torch.randn(x.shape).cuda().detach()
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0], 1, 1, 1]).cuda().detach() * t / (
                    (t ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12)  # nopep8
        ...
        step_size = self.eps * torch.ones([x.shape[0], 1, 1, 1]).to(self.device).detach() * torch.Tensor([2.0]).to(
            self.device).detach().reshape([1, 1, 1, 1])  # nopep8
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = x_adv + step_size * grad / (
                            (grad ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12)  # nopep8
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (
                        ...
                        ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt()), 0.0, 1.0)  # nopep8
                    x_adv_1 = x_adv + (x_adv_1 - x_adv) * a + grad2 * (1 - a)
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (
                            ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape).to(self.device).detach(),
                        ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12), 0.0, 1.0)  # nopep8
            ...
            with torch.no_grad():
                ...
                if counter3 == k:
                    ...
                    fl_reduce_no_impr = (~reduced_last_check) * (
                            loss_best_last_check.cpu().numpy() >= loss_best.cpu().numpy())  # nopep8
            ...
            if not cheap:
                ...
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        best_curr, acc_curr, loss_curr, adv_curr = self.attack_single_run(x_to_fool,
                                                                                          y_to_fool)  # nopep8
